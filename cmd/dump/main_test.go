...
func TestMeaning(t *testing.T) {
	tests := []line{
		...
		{"not met", 0x38E2, wire.FromServer, "01 02",
			`{"ts":1700000000006000000,"opcode":"E2 38","payload":"01 02"}`},
		...
		{"the client's", 0x3804, wire.FromClient, "c7 7c",
			`{"ts":1700000000006000000,"opcode":"04 38","payload":"c7 7c"}`},
		{"a position JSON cannot hold", 0x371A, wire.FromServer, "e4 72 00 00 00 00 c0 7f 00 00 00 00 00 00 00 00",
			`{"ts":1700000000006000000,"opcode":"1A 37","name":"Move","payload":"e4 72 00 00 00 00 c0 7f 00 00 00 00 00 00 00 00","error":"json: unsupported value: NaN"}`},
	}
